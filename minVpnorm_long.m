...
function minVpnorm_long(domain,sizex,sizey,helem,penal,rmin,ft,Sytarget,comptarget,...
...
pace = min(20,ceil(maxloop/(njumps+1)));
loop = 0;
...
while ((loop < maxloop || beta<0.99*betamaxsimul) || (maxVMscaled > Sytarget || fval(1,1) > 0))
    ...
    %% KEEP STATS
    Stats(loop,1:15+2*(m-1)) = [f0val fval' appSmax maxVMscaled beta pcgi1 pcgr1 pcgi2 pcgr2...
        ymma' zmma' lam' flag kktnorm];
end
runtime = toc;
save(filename);
end
