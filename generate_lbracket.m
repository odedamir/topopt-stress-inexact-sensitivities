function [X,T,row,col] = generate_lbracket(sizex,sizey,helem,doplot)
...
nodelist(:,1) = Xnode(:);
nodelist(:,2) = Ynode(:);
xnodeout = find(nodelist(:,1)>0.4*sizex+pad);
ynodeout = find(nodelist(:,2)>0.4*sizey+pad);
...
elemlist(:,1) = Xelem(:);
elemlist(:,2) = Yelem(:);
xelemout = find(elemlist(:,1)>0.4*sizex+pad);
yelemout = find(elemlist(:,2)>0.4*sizey+pad);
...
for e = 1:nelem
    ...
    T(e,1:4) = [n1 n2 n3 n4];
    % Assign voids
    if (x_cent<0); T(e,5:6) = [1 1]; end
    if (y_cent<0); T(e,5:6) = [1 1]; end
    if (x_cent>sizex); T(e,5:6) = [1 1]; end
    if (x_cent>0.4*sizex && y_cent>0.4*sizey); T(e,5:6) = [1 1]; end
    % Assign solids (for load)
    if (x_cent>sizex-max(pad,4) && y_cent>0.4*sizey-max(pad,4)); T(e,5:6) = [2 0]; end
    if (x_cent>sizex && y_cent>0.4*sizey-pad); T(e,5:6) = [2 1]; end
    if (x_cent>sizex-pad && y_cent>0.4*sizey); T(e,5:6) = [2 1]; end
end
